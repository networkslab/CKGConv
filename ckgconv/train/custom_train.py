    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        if is_eval_epoch(cur_epoch):
            ...
            if cfg.vp_momentum < 1:
                ...
                if len(mo_vp_loss) > 0:
                    mo_vp_loss.append(mo_vp_loss[-1] * (1 - cfg.vp_momentum) + cur_vp_loss * cfg.vp_momentum)
                else:
            ...
            if cfg.metric_best != 'auto':
                ...
                if cfg.vp_momentum < 1:
                    ...
                    if len(mo_vp_metric) > 0:
                        mo_vp_metric.append(mo_vp_metric[-1] * (1 - cfg.vp_momentum) + cur_vp_metric * cfg.vp_momentum)
                    else:
                        mo_vp_metric.append(cur_vp_metric)

                    best_epoch = getattr(np.array(mo_vp_metric), cfg.metric_agg)()
                    if cfg.mlflow.use:
                        mlflow.log_metric(f"val/mo_{m}", mo_vp_metric[-1], step=cur_epoch)
                # -----------------------------------------------
